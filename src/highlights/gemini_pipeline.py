...
from src.config import (
    CONTENT_PROFILE, GEMINI_ENABLED, GEMINI_MAX_CANDIDATES, GEMINI_MIN_SCORE,
    GEMINI_OVERLAP_THRESHOLD, GEMINI_TOP_HIGHLIGHTS, HIGHLIGHT_MODE,
    HIGHLIGHTS_DIR, TRANSCRIPT_DIR,
)
...
def _merge_judgement(candidate: dict, judgement: dict, profile_name: str, model_name: str) -> dict:
    ...
    item["gemini"] = {**judgement, "content_profile": profile_name, "ai_model": model_name}
    return item


def run_gemini_highlight_stage(video_name: str, video_path: Path, mode: str | None = None, content_profile: str | None = None) -> Path:
    ...
    for index, candidate in enumerate(candidates, start=1):
        ...
        info(f"[GEMINI {index}/{len(candidates)}] {float(candidate['start']):.1f}s -> {float(candidate['end']):.1f}s | profile={profile.name}")
        try:
            ...
            info(f"[GEMINI {index}/{len(candidates)}] score={judgement['total_score']} category={judgement['category']} cache={'hit' if from_cache else 'miss'}")
    ...
    if not judged:
        ...
        _save_json(HIGHLIGHTS_DIR / f"{video_name}_gemini_metadata.json", {
        ...
        "highlights": [
            {
                ...
                "performance": {
                    "views": None, "viewed_vs_swiped": None, "average_view_duration": None,
                    "average_percentage_viewed": None, "likes": None, "comments": None,
                    "shares": None, "subscribers_gained": None,
                },
            }
            for item in final
        ],
    ...
    if selected_mode == "compare":
        _save_json(HIGHLIGHTS_DIR / f"{video_name}_highlight_compare.json", {
            "video": video_path.name,
            "legacy": legacy_candidates,
            "gemini": final,
        })
        success(f"[COMPARE] Raport salvat; downstream rămâne pe legacy. Gemini selected {len(final)} highlights.")
        return highlights_path
