...
class GameplayTrackingState:
    ...
    current_target_source: str = "none"
    target_id: int = 0
...
def _mask_region(mask: np.ndarray, region, scale: float) -> None:
    ...
    x2 = int(
        max(
            0,
            getattr(region, "x2", getattr(region, "x", 0) + getattr(region, "w", 0)),
        )
        * scale
    )
    y2 = int(
        max(
            0,
            getattr(region, "y2", getattr(region, "y", 0) + getattr(region, "h", 0)),
        )
        * scale
    )
...
def observe_gameplay_motion(
    ...
    """Return an action-region observation from optical flow.

    This is deliberately only an observation. Target lock, hysteresis, hold and
    switch policy are handled by the stateful camera tracker below.
    """
    ...
    if previous_gray.shape != current_gray.shape:
        current_gray = cv2.resize(
            current_gray, (previous_gray.shape[1], previous_gray.shape[0])
        )
...
def _acquire_target(
    ...
    state.current_target_source = observation.source
...
def stabilize_gameplay_observations(
    ...
    """Convert noisy observations into a stable virtual-camera path."""
    if not observations:
    ...
    for observation in observations:
        ...
        if observation.scene_cut:
            ...
            state.current_target_source = "none"
        ...
        candidate_score = _clamp(
            observation.confidence + _source_bonus(observation.source), 0.0, 1.0
        )
        ...
        if valid_observation:
            if state.current_target_x is None or state.current_target_y is None:
                if candidate_score >= GAMEPLAY_TARGET_LOCK_THRESHOLD or observation.scene_cut:
                    _acquire_target(state, observation, candidate_score, now)
                    target_switch = True
            else:
                ...
                if target_distance <= same_target_radius:
                    ...
                    state.target_confidence = max(
                        candidate_score, state.target_confidence * 0.92
                    )
                    if _source_bonus(observation.source) >= _source_bonus(
                        state.current_target_source
                    ):
                        state.current_target_source = observation.source
                    state.target_locked = True
                    state.last_target_seen = now
                    ...
                    target_missing = (
                        now - state.last_target_seen > GAMEPLAY_TARGET_HOLD_TIME
                    )
                    score_switch = candidate_score >= (
                        state.target_confidence
                        + GAMEPLAY_TARGET_SWITCH_THRESHOLD
                        + GAMEPLAY_TARGET_SWITCH_PENALTY
                    )
                    semantic_upgrade = (
                        _source_bonus(observation.source)
                        >= _source_bonus(state.current_target_source) + 0.02
                        and candidate_score >= 0.80
                    )
                    reacquire_threshold = min(
                        0.90,
                        GAMEPLAY_TARGET_LOCK_THRESHOLD
                        + GAMEPLAY_TARGET_SWITCH_THRESHOLD,
                    )
                    strong_reacquire = (
                        target_missing and candidate_score >= reacquire_threshold
                    )
                    ...
                    if (
                        ...
                        # A far-away one-frame action/saliency spike does not count
                        # as seeing the locked target and cannot bypass hysteresis.
        ...
        # Critical lost-target transition: once hold time expires without a valid
        # observation, explicitly release TARGET_LOCK so gradual recentering can run.
        if (
            not valid_observation
            and state.target_locked
        ...
        holding_lost_target = (
            ...
            and not valid_observation
            and time_since_seen <= GAMEPLAY_TARGET_HOLD_TIME
        )
        if holding_lost_target:
        ...
        if observation.scene_cut:
            ...
            movement_factor = 1.0 / (
                1.0 + GAMEPLAY_MOVEMENT_PENALTY * distance_norm
            )
        ...
        debug = {
            "PRIMARY_TARGET": (
                state.current_target_source if state.target_locked else "none"
            ),
            ...
            "TARGET_CENTER": (
                ...
                if state.current_target_x is not None
                and state.current_target_y is not None
                else None
            ),
            "DESIRED_CROP_CENTER": [round(desired_x, 2), round(desired_y, 2)],
            "ACTUAL_CROP_CENTER": [
                round(state.current_crop_x, 2),
                round(state.current_crop_y, 2),
            ],
            "CROP_VELOCITY": [
                round(state.crop_velocity_x, 2),
                round(state.crop_velocity_y, 2),
            ],
            "DEAD_ZONE": [
        ...
        result.append(
            GameplayTrackPoint(
                ...
                source=(
                    "stable_gameplay_target"
                    if state.target_locked
                    else "stable_gameplay_hold"
                    if holding_lost_target
                    else "stable_gameplay_recenter"
                ),
                debug=debug,
            )
        )
