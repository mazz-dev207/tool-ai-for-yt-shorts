...
def analyze_originality(*, proposal: dict, clip: dict, transcript_text: str) -> OriginalityAnalysis:
    raw = proposal.get("originality_analysis") or {}
    transformations = [str(item).strip() for item in raw.get("recommended_transformations", []) or [] if str(item).strip()][:10]
    source_dependency = int(max(0, min(100, raw.get("source_dependency", 50) or 50)))
    context_independence = int(max(0, min(100, raw.get("context_independence", 50) or 50)))
    opening_quality = int(max(0, min(100, raw.get("current_opening_quality", 50) or 50)))
    transformation_need = int(max(0, min(100, raw.get("transformation_need", 50) or 50)))
