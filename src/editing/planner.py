...
def _visual_budget(editorial_profile: dict, configured_max: int) -> int:
    density = str(
        editorial_profile.get("editing_density", "medium") or "medium"
    ).lower()
...
def build_edit_plan(
    *,
    ...
    """Build the V3 edit plan without a dedicated Visual Hook Engine.

    Opening structure continues to come from the validated Hook/Story stages.
    Generic semantic visual events may still be generated by visual_grammar,
    but no extra camera-whip/show-result-first/source-native hook layer is
    selected, injected, or forced here.
    """
