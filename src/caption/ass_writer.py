...
class ASSWriter:
    ...
    def write_events(
        ...
        for group in groups:
            ...
            events = build_word_events(group)
            ...
            for event in events:
                ...
                # \q0 keeps ASS smart wrapping active inside the configured
                # left/right safe margins. Alignment comes from the style
                # (default 5 = exact center of the 1080x1920 canvas).
                file.write(
                    "Dialogue: 0,"
                    f"{ass_time(start)},"
                    f"{ass_time(end)},"
