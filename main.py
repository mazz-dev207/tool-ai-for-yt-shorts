...
from src.config import (
    INPUT_DIR, TRANSCRIPT_DIR, HIGHLIGHTS_DIR, OUTPUT_DIR, SUBTITLES_DIR,
    FINAL_DIR, TEMP_DIR, RETENTION_ENABLED, HOOK_OPTIMIZER_ENABLED,
    HIGHLIGHT_MODE, CONTENT_PROFILE,
)
...
def parse_args():
    ...
    parser.add_argument("--highlight-mode", choices=["legacy", "gemini", "compare"], default=HIGHLIGHT_MODE, help="legacy, gemini sau compare; implicit vine din .env/config")
    parser.add_argument("--content-profile", choices=["auto", "gaming", "entertainment", "podcast", "reaction", "general"], default=CONTENT_PROFILE, help="Profil folosit de discovery, Gemini, Hook, V3 editorial reasoning și SmartCrop.")
    parser.add_argument("--smartcrop-mode", choices=["auto", "gameplay-webcam", "gameplay-only"], default="auto", help="Politica SmartCrop. gameplay-webcam forțează contractul webcam sus 35-40% / gameplay jos 60-65%.")
    parser.add_argument("--v3-mode", choices=["auto", "on", "off"], default="auto", help="auto folosește V3_ENABLED din mediu; on forțează V3; off rulează pipeline-ul V2.")
...
def main():
    ...
    v3_enabled = V3_ENABLED if args.v3_mode == "auto" else args.v3_mode == "on"
    ...
    started = time.time()
    for folder in [TRANSCRIPT_DIR, HIGHLIGHTS_DIR, OUTPUT_DIR, SUBTITLES_DIR, FINAL_DIR, TEMP_DIR]:
        clean_folder(folder)
    ...
    _, timings["candidate_discovery"] = timed_step(generate_candidates, video_name, args.highlight_mode in {"gemini", "compare"}, args.content_profile)
    _, timings["gemini_judge"] = timed_step(run_gemini_highlight_stage, video_name, video_path, args.highlight_mode, args.content_profile)
    ...
    if HOOK_OPTIMIZER_ENABLED:
        ...
        _, timings["hook_optimizer"] = timed_step(optimize_hooks, video_name, video_path, args.content_profile)
    else:
    ...
    if v3_enabled:
        ...
        _, timings["v3_editorial"] = timed_step(run_v3_editorial_stage, video_name, video_path, args.content_profile)
    else:
        info("6/10 V3 dezactivat; folosesc comportamentul V2.")
        timings["v3_editorial"] = 0.0

    info("7/10 SmartCut / asamblare timeline...")
    _, timings["cut"] = timed_step(cut_v3 if v3_enabled else cut, video_name)

    info("8/10 Generare subtitrări + karaoke...")
    caption_engine = CaptionEngine()
    _, timings["captions"] = timed_step(caption_engine.generate, video_name)

    clips = sorted(OUTPUT_DIR.glob("clip_*.mp4"), key=lambda path: int(path.stem.split("_")[1]))
    if not clips:
        raise RuntimeError("Nu au fost generate clipuri.")

    info(f"9/10 SmartCrop + render {len(clips)} clipuri | SmartCrop mode={args.smartcrop_mode}")
    render_started = time.time()
    rendered_outputs = [render(clip.stem, args.content_profile) for clip in clips]
    ...
    if v3_enabled:
        ...
        for index, output in enumerate(rendered_outputs, start=1):
            post_process_v3(video_name=video_name, clip_index=index, rendered_path=Path(output))
        timings["semantic_post"] = time.time() - started
    else:
        info("10/10 Semantic V3 effects dezactivate.")
        timings["semantic_post"] = 0.0

    total_elapsed = time.time() - pipeline_started
    print()
    success("AI Shorts V3 pipeline terminat cu succes!" if v3_enabled else "Pipeline V2 terminat cu succes!")
    print()
    print(f"Clipuri generate: {len(clips)}")
    print(f"Rezultate finale: {FINAL_DIR}")
    print()
    print("--- TIMPI PIPELINE ---")
    for key in ["cleanup", "transcription", "chunking", "candidate_discovery", "gemini_judge", "retention", "hook_optimizer", "v3_editorial", "cut", "captions", "render", "semantic_post"]:
